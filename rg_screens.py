...
_MENU_BUTTON_CACHE = {"size": None, "surface": None, "path": None}
...
def _find_menu_button_path():
    for path in MENU_BUTTON_PATHS:
        if path.exists():
            return path
    return None


def _load_menu_button_texture(size):
    path = _find_menu_button_path()
    cache_key = (size, str(path) if path else None)
    if _MENU_BUTTON_CACHE["size"] == cache_key:
        return _MENU_BUTTON_CACHE["surface"]
    if not path:
        _MENU_BUTTON_CACHE["size"] = cache_key
        _MENU_BUTTON_CACHE["surface"] = None
        _MENU_BUTTON_CACHE["path"] = None
        return None
    try:
    ...
    _MENU_BUTTON_CACHE["size"] = cache_key
    _MENU_BUTTON_CACHE["surface"] = texture
    _MENU_BUTTON_CACHE["path"] = path
    return texture


def _draw_menu_button(screen, font, mouse, button):
    hovered = button.rect.collidepoint(mouse)
    texture = _load_menu_button_texture(button.rect.size)
    if texture:
    ...
    if button.text:
        ...
        label = font.render(button.text, True, TEXT if not hovered else (255, 232, 170))
...
def draw_menu(screen, title_font, font, mouse):
    ...
    if background:
        ...
        screen.fill(BG)
    start_y = int(clamp(SCREEN_HEIGHT * 0.39, 300, 355))
    buttons = vertical_buttons(
        [("Nowa gra", "new"), ("Multiplayer", "multi"), ("Wyjscie", "exit")],
        start_y,
        width=360,
        height=50,
        gap=10,
    ...
    for button in buttons:
        _draw_menu_button(screen, font, mouse, button)
    return buttons
