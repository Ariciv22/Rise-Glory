...
def draw_player_count(screen, title_font, font, mouse):
    ...
    buttons = []
    start_x = SCREEN_WIDTH / 2 - 3 * 110 - 20
    for index in range(6):
        rect = pygame.Rect(start_x + (index % 3) * 230, 280 + (index // 3) * 110, 200, 76)
        button = Button(str(index + 1), f"count:{index + 1}", rect)
        button.draw(screen, font, mouse)
...
def draw_player_config(screen, title_font, font, small_font, mouse, player_index, player_count, world_name, selected_archetype_id, used_archetypes):
    screen.fill(BG)
    ...
    for idx, hero in enumerate(HERO_ARCHETYPES):
        ...
        selected = hero["id"] == selected_archetype_id
        ...
        for line in wrap(small_font, f"Start: {hero['basic_item']} + {hero['class_item']}", card_w - 36)[:2]:
            ...
            y += 20
        buttons.append(Button("", f"archetype:{hero['id']}", rect))

    bottom_y = 780
    random_button = Button("Losowy bohater", "random", (SCREEN_WIDTH / 2 - 470, bottom_y, 270, 54))
    custom_button = Button("Stworz bohatera", "custom", (SCREEN_WIDTH / 2 - 180, bottom_y, 300, 54))
    next_button = Button("Zatwierdz gracza", "confirm", (SCREEN_WIDTH / 2 + 140, bottom_y, 330, 54))
    back = Button("Powrot", "back", (SCREEN_WIDTH / 2 - 120, 860, 240, 48))
    for button in [random_button, custom_button, next_button, back]:
        button.draw(screen, font, mouse)
        buttons.append(button)

    ready = bool(world_name.strip()) and selected_archetype_id is not None
    if not ready:
        warning = "Wpisz imie i wybierz archetyp."
        screen.blit(small_font.render(warning, True, (235, 170, 95)), (SCREEN_WIDTH / 2 + 150, bottom_y - 28))
    return buttons


def draw_custom_hero(screen, title_font, font, small_font, mouse, player_index, player_count, world_name, base_hero, stats):
    screen.fill(BG)
    draw_title(screen, title_font, font, f"Stworz bohatera - Gracz {player_index + 1}/{player_count}", f"Wyglad i ekwipunek: {base_hero['name']}")
    remaining = 12 - sum(stats.values())
    screen.blit(font.render(f"Pozostale punkty: {remaining}", True, TEXT), (SCREEN_WIDTH / 2 - 120, 170))
    screen.blit(small_font.render(f"Imie: {world_name}", True, MUTED), (SCREEN_WIDTH / 2 - 120, 208))
    ...
    for idx, stat in enumerate(STAT_NAMES):
        ...
        minus = Button("-", f"stat:{stat}:-1", (row.right - 210, row.y + 8, 52, 42))
        plus = Button("+", f"stat:{stat}:1", (row.right - 70, row.y + 8, 52, 42))
...
def draw_hero_select(screen, title_font, font, small_font, mouse):
    screen.fill(BG)
    draw_title(screen, title_font, font, "Wybierz bohatera", "Ten ekran pozostaje tylko dla zgodnosci prototypu")
    buttons = []
