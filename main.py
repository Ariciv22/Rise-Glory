...
def player_name_input_rect():
    width = min(540, max(360, SCREEN_WIDTH - 120))
    return pygame.Rect(SCREEN_WIDTH / 2 - width / 2, 170, width, 58)
...
def main():
    global SCREEN_WIDTH, SCREEN_HEIGHT
    pygame.init()
    sync_screen_size((SCREEN_WIDTH, SCREEN_HEIGHT))
    activate_text_input()
    ...
    while running:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                sync_screen_size((event.w, event.h))
                screen = create_window(False)
